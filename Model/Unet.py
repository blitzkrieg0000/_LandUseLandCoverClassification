...
class CustomUnet(BaseModel):
	def __init__(self, in_channels=13, out_channels=9, init_features=32):
		...
		self.bottleneck = CustomUnet._block(features * 8, features * 16, name="bottleneck")
...
if "__main__" == __name__:
	model = CustomUnet(in_channels=13, out_channels=9).to(DEVICE)
	dummy = torch.rand(1, 13, 64, 64).to(DEVICE)
	output = model(dummy)
	print(output.shape)
	print(summary(model))